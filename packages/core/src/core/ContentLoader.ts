import { Scene, SceneId, ScenesRegistry, SceneDefinition } from '../types';
...
export class ContentLoader {
    ...
    public async loadScene(sceneId: SceneId): Promise<Scene> {
        ...
        if (typeof sceneDefOrImport === 'function') {
            loadPromise = sceneDefOrImport().then((module: SceneModule) => {
                const scene = 'default' in module ? module.default : module;
    ...
    public async preloadScenes(sceneIds?: SceneId[]): Promise<void> {
        const idsToLoad = sceneIds || this.getSceneIds();
